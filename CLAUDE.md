...
3. **Quiz Logic**:
   ...
   - 記述で出すかは**選ばれた解き方だけ**で決まる。単語の状態 (`vocabularyType === 'active'` / `status === 'active'`) から記述に切り替えてはいけない —— 四択を選んだのに一部の語だけ入力欄になる、が元の不具合。語順クイズ (`isWordOrderEligible`) だけは問題の作りが別物なので、どちらを選んでも語順のまま出る
