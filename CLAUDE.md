...
3b. **目標ページ (`/goal`, `src/app/goal/page.tsx`)**: Pro の下部バー2番目のタブ（旧「単語」`/words` の位置。`/words` 自体は残っているがナビからは外した）。目標は**「どの単語帳を・いつまでに」を単語帳の選択で指定**し、端末の localStorage に持つ（`src/lib/goal/study-goal.ts`、1日の復習上限と同じ扱いで端末間には同期しない）。載せるのは 残り日数バナー / 月間カレンダー（月曜はじめ・今日・目標日・学習した日）/ **今日の10問**（目標の単語帳だけから `/quiz/[projectId]?count=10`）/ **今日復習しておきたい単語**（SM-2 の復習期限、全単語帳横断 `/quiz/all?review=1`）。後者2つは別物で混ぜない。ホームのショートカットグリッドからは「今日の復習」「保存済み単語」タイルを外した（デスクトップ版 `DesktopHome` は据え置き）
