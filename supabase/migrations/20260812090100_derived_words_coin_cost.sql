...
--   manual-add              = 1 coin / word
--
-- Why the two rates differ: the scan surcharge is charged once for a whole
-- scan that may yield dozens of words, while manual add charges per word.
-- Pricing the scan surcharge at 1 would make a 20-word scan cheaper than
-- adding a single word by hand.
--
-- Policy (same as manual-add morphology): coins are Pro-only, but manual add
-- itself is a core Free-tier feature and must never be blocked. A Free user, or
-- a Pro user who is out of coins, gets `allowed:false` here (no charge, no
-- error) and the caller simply skips attaching derived words — the word is
-- still saved. Charging is best-effort and success-gated: the route only calls
-- this after displayable derived words were produced.
--
...
-- dispatch becomes ambiguous. Callers that omit p_include_derived_words keep
...
ALTER TABLE public.coin_transactions
  ADD CONSTRAINT coin_transactions_type_check CHECK (type IN (
    ...
    'manual_derived_words_consume'
  ));

-- ============================================
-- 2. scan_coin_cost / consume_scan_coins with the derived-words surcharge
-- ============================================

DROP FUNCTION IF EXISTS public.consume_scan_coins(TEXT[], INTEGER, UUID, BOOLEAN);
DROP FUNCTION IF EXISTS public.scan_coin_cost(TEXT[], INTEGER, BOOLEAN);
...
BEGIN
  ...
  RETURN v_cost + (p_image_count - 1)
    ...
    + (CASE WHEN COALESCE(p_include_derived_words, FALSE) THEN 2 ELSE 0 END);
END;
$$;
...
BEGIN
  ...
  v_cost := public.scan_coin_cost(
    p_modes, p_image_count, p_include_morphology, p_include_derived_words
  );
  ...
  VALUES
    (v_user_id, 'scan_consume', -v_from_monthly, -v_from_purchased,
     ...
     jsonb_build_object(
       ...
       'include_derived_words', COALESCE(p_include_derived_words, FALSE),
       'month_key', v_month
     ));
...
REVOKE ALL ON FUNCTION public.consume_scan_coins(TEXT[], INTEGER, UUID, BOOLEAN, BOOLEAN) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.consume_scan_coins(TEXT[], INTEGER, UUID, BOOLEAN, BOOLEAN) TO authenticated;

-- ============================================
-- 3. consume_manual_derived_words_coins (authenticated)
-- ============================================

CREATE OR REPLACE FUNCTION public.consume_manual_derived_words_coins(
...
BEGIN
  ...
  -- skips derived words and still saves the word.
  IF NOT v_is_pro THEN
    RETURN jsonb_build_object(
      'allowed', false,
      'requires_pro', true,
  ...
  -- DERIVED_WORDS_COIN_COST = 1 per word (mirrored in src/lib/coins/rates.ts)
  v_cost := p_count * 1;
  ...
  INSERT INTO public.coin_transactions
    (user_id, type, monthly_amount, purchased_amount, monthly_after, purchased_after, metadata)
  VALUES
    (v_user_id, 'manual_derived_words_consume', -v_from_monthly, -v_from_purchased,
     v_balance.monthly_coins, v_balance.purchased_coins,
...
REVOKE ALL ON FUNCTION public.consume_manual_derived_words_coins(INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.consume_manual_derived_words_coins(INTEGER) TO authenticated;
