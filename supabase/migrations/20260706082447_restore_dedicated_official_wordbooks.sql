-- Restore dedicated official wordbook storage.
--
-- Official source content must not live in a real user's projects. Older
-- migrations briefly marked normal projects as official sources; keep the
-- compatibility columns, but clear their official metadata and use these
-- dedicated tables for onboarding imports.
...
CREATE INDEX IF NOT EXISTS idx_official_wordbooks_level_default
  ON public.official_wordbooks (eiken_level, is_default DESC, sort_order, created_at)
  WHERE is_active;
...
CREATE INDEX IF NOT EXISTS idx_official_wordbook_words_lexicon_entry_id
  ON public.official_wordbook_words (lexicon_entry_id)
  WHERE lexicon_entry_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_official_wordbook_words_lexicon_sense_id
  ON public.official_wordbook_words (lexicon_sense_id)
  WHERE lexicon_sense_id IS NOT NULL;
...
WHERE official_slug IS NOT NULL
   OR official_title IS NOT NULL
   OR official_description IS NOT NULL
   OR official_eiken_level IS NOT NULL
   OR official_is_default
   OR official_is_active;
