...
CREATE TABLE IF NOT EXISTS public.word_translations (
  ...
  lexicon_sense_id uuid NULL REFERENCES public.lexicon_senses(id) ON DELETE SET NULL,
...
SELECT
  ...
  w.updated_at
FROM public.words w
WHERE public.normalize_lexicon_translation_key(w.japanese) IS NOT NULL
