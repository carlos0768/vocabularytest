...
test('normalizeGeminiModel upgrades deprecated flash requests to gemini-2.5-flash', () => {
  assert.equal(normalizeGeminiModel('gemini-2.0-flash-001'), DEFAULT_GEMINI_GENERATION_MODEL);
...
test('normalizeGeminiModel preserves supported pro models', () => {
  ...
  assert.equal(normalizeGeminiModel('gemini-2.0-flash'), DEFAULT_GEMINI_GENERATION_MODEL);
});
