...
> Current UI rescue note: この文書は旧 `codex/prelaunch-safety-baseline` 上でのscan process分割完了記録を元にしています。記載されたhelperとtestは最新UIブランチへ再移植済みです。最終状態は `docs/maintenance/AI_HANDOFF.md` と `docs/maintenance/TASKS.md` を正としてください。
