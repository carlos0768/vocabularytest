...
> Current UI rescue note: この文書は旧 `codex/prelaunch-safety-baseline` 上でのscan process分割完了記録です。最新UIブランチでは、ここに書かれたhelperとtestを最新 `origin/main` のrouteへ再実装するための仕様として扱います。再移植の進捗は `docs/maintenance/TASKS.md` を正としてください。
