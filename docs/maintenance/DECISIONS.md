...
## 2026-05-09: 旧保守性向上作業は最新UIブランチへ再移植する

判断:

- 旧 `codex/prelaunch-safety-baseline` は古い `main` 由来なので、公開前作業の土台として使わない。
- 旧ブランチの56コミットは一括cherry-pickせず、maintenance docsと旧コミットを仕様として参照しながら、最新 `origin/main` 起点の `codex/prelaunch-safety-baseline-current-ui` へ1領域ずつ再実装する。
- 旧docs内の「完了済み」は旧ブランチ上の履歴として残し、最新UIブランチでの完了状態は `AI_HANDOFF.md` と `TASKS.md` で管理する。
- `src/components/redesign/**` と `src/app/globals.css` は原則変更せず、Home/Project/Quizを触る場合も最新UIの見た目・文言・レイアウトを正とする。

理由:

- 旧ブランチをそのまま使うと最新UIを巻き戻すため、保守性改善の価値より公開事故リスクが大きい。
- ただし旧ブランチで作ったcontract test、helper抽出、runbook整理は保守性向上として有効なので、最新コードへ救出する価値がある。
- docsを先に救出して現在地を固定すると、次のAIが旧完了記録を最新ブランチの完了状態と誤読しにくくなる。

## 2026-05-16: Duolingo形式クイズの単語プレートから発音記号を削除する

判断:

- `src/app/quiz/[projectId]/page.tsx` の単語表示プレート内にある発音記号表示 (`word.pronunciation`) を削除する。
- 対象: 約988行目の `<div className="mb-2 font-mono text-[11px] ...">` ブロック。
