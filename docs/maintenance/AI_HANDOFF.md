...
- `npm run build`: 成功
- `npm test`: 成功。346 tests pass。Web/shared通常testの固定リストを実行。固定リストは自動発見ではなく、通過確認済みtestだけを含める
...
3. Home巨大ファイルの棚卸しは [`HOME_PAGE_AUDIT.md`](HOME_PAGE_AUDIT.md) に完了済み。Home表示selector、Home専用sessionStorage key、scan job local notification message builderの純粋helper化も完了済み。Home scan client flowの棚卸しは [`HOME_SCAN_FLOW_AUDIT.md`](HOME_SCAN_FLOW_AUDIT.md) に完了済み。次にHome実装へ進む場合は、`immediate scan progress step builder` をpure helperへ出す小タスクから始める
