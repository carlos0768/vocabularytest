...
3. Home巨大ファイルの棚卸しは [`HOME_PAGE_AUDIT.md`](HOME_PAGE_AUDIT.md) に完了済み。Home表示selector、Home専用sessionStorage key、scan job local notification message builderの純粋helper化も完了済み。次にHome実装へ進む場合は、immediate/background scan client flowを1責務ずつ進める
