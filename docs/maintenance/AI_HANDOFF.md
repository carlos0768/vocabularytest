...
- `npm run build`: 2026-05-09 Home immediate scan progress helper作業時点では失敗。今回未変更の `src/lib/home/home-session-storage.ts:75` で `Object is of type 'unknown'`。sessionStorage helperは今回の禁止範囲だったため未修正
...
- `npm run verify`: 直近の既知成功記録はあるが、2026-05-09 Home immediate scan progress helper作業では `build` が上記既存type errorで失敗したため通過していない
