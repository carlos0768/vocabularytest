...
- [ ] `package-lock.json` と実際のinstall結果を一致させる
  - 現状: local `node_modules` と lockfile で `next`, `uuid`, `protobufjs`, `fast-jwt`, `undici` などがズレている
  - 成功条件: clean install後に `npm run build`, `npm test`, `npm run security:deps` が通る
- [ ] dependency auditを通す
  - 現状: `npm run security:deps` が high=3 / critical=2 で失敗
  - 成功条件: `npm audit --omit=dev --audit-level=high` 相当が通る
