...
| `public/offline.html`, `public/offline-viewer.js` | Offline navigation fallback + the wordbook viewer it renders. Precached by `sw.js`, so they must stay dependency-free plain HTML/JS (no imports, no build output). | Manual edits only; pure helpers are covered by `src/lib/offline/offline-viewer.test.ts` |
