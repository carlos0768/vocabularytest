...
| コスト・abuse 対策 | 8/10 | 認証必須化、日次 AI usage limit、API cost event、Cloud Run scale cap、gateway/fallback cap、GCP monthly budget がある。 |
...
本番 smoke と GCP billing safety audit が通ったため、初回一般公開 v1.0 は「ユーザーを増やし始められる状態」と判断します。GCP 高額請求リスクの詳細は [`gcp-billing-safety-audit-2026-06-13.md`](gcp-billing-safety-audit-2026-06-13.md) を正とします。
...
| `npm run build` | 成功。 |
| `npm test --prefix cloud-run-scan` | 成功。23 tests pass。 |
...
完了。詳細は [`gcp-billing-safety-audit-2026-06-13.md`](gcp-billing-safety-audit-2026-06-13.md) に記録しました。

現在の主な guardrail:

- Cloud Run `scanvocab-ai-gateway`: max instances 3、min instances 0、concurrency 10。
- `AUTH_TOKEN` / `OPENAI_API_KEY`: Secret Manager 参照。
- fallback: `gpt-4o-mini`、100 calls / 300 円 estimate per day。
- gateway: 300 calls / 900 円 estimate per day。
- GCP Billing budget: 本番 project scoped、5,000 円 / month。
