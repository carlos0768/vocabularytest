...
5. 修復は過去migration編集ではなく、新しいidempotent migrationで行い、最後にschema cache reloadを通知する。

```sql
NOTIFY pgrst, 'reload schema';
```

6. 修復後に、該当APIでrelation embedあり/なし両方の挙動を確認する。critical pathはrelation embedだけに依存させない。
