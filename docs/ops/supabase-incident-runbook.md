...
- Vercel Runtime Logs
  ...
  - `Invalid API key`
  - `/api/health` の `Database unreachable`
- Vercel Project / Environment Variables
  - production URL がどの Vercel project を指しているか
  - GitHub check / preview deployment の project と production alias の project が同じか
  - Supabase / Cloud Run / worker token にリテラル `\n` が混入していないか
...
3. Vercelの本番aliasが指すprojectを確認する。

```bash
vercel inspect https://englishvo.vercel.app
```

4. Vercel production envを値非表示で確認し、単一行tokenにリテラル `\n` が混入していないか確認する。
5. Supabase DashboardでProject statusとAPI availabilityを確認する。
6. Supabase LogsでPostgres / API / Authのエラーを確認する。
7. Table Editorで対象テーブルの直近行と対象ユーザー行が存在するか確認する。
8. SQL Editorでは読み取り専用SQLだけを実行し、テーブル存在、RLS有効状態、policy、直近データを確認する。
9. migration事故が疑われる場合、`supabase/migrations/` の最新ファイル名と本番に適用済みのschema状態を照合する。
10. 原因が未確定のまま、本番テーブルのUPDATE、DELETE、DROP、ALTER、policy変更をしない。
...
for (const line of raw.split(/\n/)) {
  ...
  if (!/SUPABASE|CLOUD_RUN|WORKER_TOKEN|API_KEY|SECRET/.test(name)) continue;
  console.log(`${name}: literal_backslash_n=${value.includes('\\n') ? 'yes' : 'no'}, actual_newline=${value.includes('\n') ? 'yes' : 'no'}`);
}
NODE
rm -f .vercel-env-production.local
```

`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `CLOUD_RUN_URL`, `CLOUD_RUN_AUTH_TOKEN` のような単一行値で `literal_backslash_n=yes` が出た場合、runtimeではその2文字が値の一部として扱われ、Supabase `Invalid API key` や Cloud Run認証失敗の原因になります。修正後も既存deploymentは古いenv snapshotを持つため、反映には再deployが必要です。
...
## PostgREST schema cache / migration drift確認

`schema cache`, `PGRST200`, `PGRST204`, `Could not find a relationship`, `Could not find the '<column>' column` が出ている場合は、DBテーブル/FKの存在確認だけでは不十分です。PostgREST APIのschema cacheとmigration historyも確認します。

1. remote migration historyを確認する。

```bash
supabase migration list --linked
```

2. `Local` に値があり `Remote` が空の行を確認する。
   - 新しいapp codeがそのmigrationの列やrelationを使っている場合、本番では書き込み/読み込みが失敗する。
   - 既により新しいmigrationがremote適用済みでも、古いmigrationがremote未適用ならschema driftとして扱う。
3. SQL Editorで対象列とFKを読み取り確認する。
4. FKが存在するのにrelation embedが失敗する場合は、PostgREST schema cache reloadが必要な可能性が高い。
5. app codeがinsert/selectする列をgrepし、migrationで実際に作られているか確認する。migration内で存在チェックしているだけの列は「作成済み」と見なさない。
6. 修復は過去migration編集ではなく、新しいidempotent migrationで行い、最後にschema cache reloadを通知する。

```sql
NOTIFY pgrst, 'reload schema';
```

7. 修復後に、該当APIでrelation embedあり/なし両方の挙動を確認する。critical pathはrelation embedだけに依存させない。
