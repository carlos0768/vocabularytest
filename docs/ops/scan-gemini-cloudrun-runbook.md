...
- Gateway cap: Gemini / OpenAI を問わず Cloud Run gateway 全体に日次capをかける。Firestore guard有効時は全Cloud Run instance共通で判定する。
...
- Amount: 5,000 JPY / month
- Pub/Sub topic: `merken-budget-guard`
- Function: `merken-budget-guard`
- 停止条件: actual / forecast のどちらかが 90% 以上。
- 停止時: `ops/aiGatewayGuard.disabled=true` になり、gatewayはprovider呼び出し前に429を返す。
...
- gateway cap到達（`gateway-cost-cap-reached`, `gateway-budget-guard-blocked`）
...
- [ ] Cloud Run `/health` の `gatewayGuardStore` が `firestore`
