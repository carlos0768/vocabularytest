...
2026-06-13 時点の `merken.jp` 本番運用では、GCP 高額請求を防ぐための最低限の防御線は入っています。
...
- Cloud Run gateway 全体の日次 cap は 300 calls / 900 円。cap 到達時は provider 呼び出し前に 429。
...
この状態なら、初回の無料一般公開は「ユーザーを増やし始めてよい」状態です。ただし、Budget は停止装置ではなく通知装置です。月 5,000 円 budget に近づいたら、gateway cap を上げる前に実請求と `/ops/api-costs` を比較してください。
...
1. Budget は必須。ただし停止装置ではない。
   ...
   - したがって MERKEN では budget alert に加えて、アプリ側・Cloud Run 側に provider 呼び出し前の cap を置く。
...
`FALLBACK_*` は Gemini 障害時の OpenAI fallback だけを止めます。`GATEWAY_*` は Gemini / OpenAI を問わず Cloud Run gateway 全体を provider 呼び出し前に止めます。
...
| Gateway daily cap | Cloud Run 経由 provider 呼び出しの日次暴走 | 300 calls / 900 円 estimate |
...
1. Budget は停止装置ではない。
   ...
   - 完全停止が必要なら Pub/Sub budget notification + Cloud Run function で billing disable を作る。ただし billing disable は project の全サービス停止を伴うため、通常運用の初期防御にはしない。

2. Gateway cap は Cloud Run instance ごとの in-memory cap。
   - `max instances=3` なので通常時の上振れは最大 3 instance 分に抑えられる。
   - revision restart や新 revision deploy で in-memory counter はリセットされる。
   - 高精度な global hard cap が必要になったら、Supabase / Memorystore / Firestore など永続 store で global counter を実装する。
...
Cloud Run gateway を止めたい場合:

```bash
gcloud run services update scanvocab-ai-gateway \
  --region asia-northeast1 \
  --update-env-vars GATEWAY_CALLS_DAILY_CAP=0,GATEWAY_COST_DAILY_CAP_YEN=0
```
