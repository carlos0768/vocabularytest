...
| コスト | AI call、token、Cloud Run fallback、Vercel usage | `/ops/api-costs`、OpenAI/Gemini/GCP/Vercel Billing |
...
6. Cloud Run / GCP
   ...
   - Gemini quota と fallback OpenAI cost cap を見る。
...
4. Vercel / GCP / OpenAI / Gemini の請求見込みを確認する。
...
- AI cost が原因なら AI usage limit を下げる、該当機能を env で止める、Cloud Run fallback cap を下げる。
...
3. Cloud Run 5xx / 429 / fallback cap reached の通知
