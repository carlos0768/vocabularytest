...
## Coin System (scan consumption)

`COIN_SYSTEM_ENABLED=true` switches Pro scanning from unlimited to coin-based.

```text
scan route (/api/extract | /api/scan-jobs/create | /api/scan-jobs)
  -> consumeScanGate (src/lib/coins/scan-gate.ts)
       flag off -> check_and_increment_scan_batch (legacy, unchanged)
       flag on  -> consume_scan_coins RPC
                     - is_active_pro check (provider-agnostic)
                     - lazy monthly grant (300 coins, JST calendar month, no rollover)
                     - atomic deduction: monthly coins first, then purchased
                     - coin_transactions ledger row (scan_job_id pre-generated by the route)
                     - daily_scan_usage still incremented (ops continuity)
  -> total scan failure -> refund_scan_coins RPC (service role, idempotent per job)

coin pack purchase (web only, Pro only)
  /coins page -> POST /api/coins/checkout -> Stripe Checkout (mode=payment, card + PayPay)
  -> checkout.session.completed webhook (purpose=coin_pack branch)
     + GET /api/coins/checkout/confirm (success-page immediate credit)
  -> credit_coin_pack RPC — idempotent via unique (provider, external_ref)
```
...
- **Phase 2a (landed)**: gateway is GMO PG. `src/lib/paypay/gmo/` holds the transport (form-urlencoded, and business errors arrive as HTTP 200 so success is decided by `ErrCode`/`ErrInfo` alone), and the notification trust model. **GMO result notifications carry no signature** — no HMAC like Stripe (INV-05), no JWS like App Store — so `/api/subscription/paypay/notifications` never trusts the posted state: source-IP allowlist (deny-all when unconfigured) → `claim_webhook_event` → **re-query GMO** → apply only what the re-query confirms. See `docs/paypay-gmo-integration.md`.
- **Phase 2b (blocked on the GMO interface spec)**: every spec-dependent constant is isolated in `src/lib/paypay/gmo/spec.ts`; until the re-query API is filled in, the notification route fails closed and cannot activate Pro. Still gated behind `PAYPAY_SUBSCRIPTION_ENABLED` (default off).
