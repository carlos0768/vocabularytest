...
日次上限は `AI_LIMIT_VOICE_QUIZ_FREE_DAILY` / `AI_LIMIT_VOICE_QUIZ_PRO_DAILY` (feature-usageの仕組みを流用) で管理しており、**問題数ではなく認識API呼び出し回数**でカウントされる。したがって試行回数3回設定なら1日に解ける問題数はその分減る（上限突破によるコスト超過は起きない）。Free上限30回でも 30×15秒=7.5分/日 で月の無料枠(60分)に収まる規模、Pro上限300回でも1日75分・$0.03/日程度であり、コストリスクは小さい。
