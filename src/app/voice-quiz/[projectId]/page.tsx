...
import { speakAndWait, speakEnglish } from '@/lib/speech';
...
/** AI生成に失敗した単語向けの機械的な出題文。ネットワーク不通時でも出題を止めないための保険。 */
function buildFallbackVoiceQuizPrompt(word: Word): string {
  return `「${word.japanese}」という意味の単語、英語で何と言う?`;
}

const normalize = (s: string) =>
  s.toLowerCase().trim().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, ' ');

type Phase = 'narrating' | 'listening' | 'grading' | 'answered';
...
export default function VoiceQuizPage() {
  ...
  // 単語の読み込み + 出題文(voiceQuizPrompt)の準備。
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const selected = sortWordsByPriority(loaded).slice(0, Math.min(loaded.length, DEFAULT_COUNT));
        const prepared = await ensureVoiceQuizPrompts(selected, repository);
        setWords(prepared);
      } catch {
  ...
  const handleAnswer = useCallback(
    async (transcript: string, apiErrored: boolean) => {
      ...
      const normalizedAnswer = normalize(transcript);
      const normalizedExpected = normalize(word.english);
      const correct = !!transcript && normalizedAnswer === normalizedExpected;
      ...
      // 正解の発音をフロントエンドTTSで読み上げる(出題文の朗読 + 単語音声の組み合わせ)。
  ...
  const startQuestion = useCallback(() => {
    ...
    void (async () => {
      await speakAndWait(word.voiceQuizPrompt || buildFallbackVoiceQuizPrompt(word), 'ja');
      if (questionRunRef.current !== run || !streamRef.current || !recordingRef.current) return;
      ...
      recorder.onstop = () => {
        void (async () => {
          ...
          try {
            ...
            handleAnswer(typeof data.transcript === 'string' ? data.transcript : '', false);
          } catch {
            if (questionRunRef.current === run) handleAnswer('', true);
          }
        })();
      };
  ...
  }, [phase, stopRecordingAndGrade]);

  // 準備が整ったら最初の問題を開始する。
  useEffect(() => {
    if (!loading && words.length > 0 && setupState === 'ready' && currentIndex === 0 && questionRunRef.current === 0) {
      startQuestion();
    }
  }, [loading, words.length, setupState, currentIndex, startQuestion]);
      ...
      <div className="h-screen flex items-center justify-center bg-[var(--color-background)] overflow-hidden">
        <div className="text-center">
          ...
          <p className="text-[var(--color-muted)]">
            {setupState === 'checking' ? 'マイクを確認中...' : '問題を準備しています...'}
          </p>
        </div>
      </div>
    );
...
async function ensureVoiceQuizPrompts(
  words: Word[],
  repository: ReturnType<typeof getRepository>,
): Promise<Word[]> {
  const missing = words.filter((w) => !w.voiceQuizPrompt?.trim());
  if (missing.length === 0) return words;

  const promptById = new Map<string, string>();
  for (let i = 0; i < missing.length; i += PROMPT_BATCH_SIZE) {
    const chunk = missing.slice(i, i + PROMPT_BATCH_SIZE);
    try {
      const response = await fetch('/api/generate-voice-quiz-prompt', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          words: chunk.map((w) => ({
      ...
      if (response.ok && data.success && Array.isArray(data.results)) {
        for (const result of data.results as Array<{ wordId: string; prompt: string }>) {
          if (result.wordId && result.prompt) promptById.set(result.wordId, result.prompt);
        }
      }
    } catch {
      // ネットワーク不通時はフォールバック出題文を使うため無視する。
    }
  }

  const prepared = words.map((w) => {
    if (w.voiceQuizPrompt?.trim()) return w;
    const prompt = promptById.get(w.id) || buildFallbackVoiceQuizPrompt(w);
    return { ...w, voiceQuizPrompt: prompt };
  });

  await Promise.all(
    prepared
      .filter((w, i) => w.voiceQuizPrompt !== words[i]?.voiceQuizPrompt)
      .map((w) => repository.updateWord(w.id, { voiceQuizPrompt: w.voiceQuizPrompt }).catch(() => {})),
  );
