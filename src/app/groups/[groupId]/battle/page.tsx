...
export default function GroupBattlePage() {
  ...
  useEffect(() => {
    ...
    loadGroupOverview(groupId, (payload) => {
      if (!cancelled) setGroup(payload.group);
    }).catch(() => {
      // グループ名はヘッダの表示だけに使うので、失敗しても対戦自体は続けられる。
    });
  ...
  const startMatching = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/battle/match', {
        ...
        body: JSON.stringify({ groupId, projectId, questionCount, roundDurationMs }),
      });
  ...
  }, [groupId, projectId, questionCount, roundDurationMs, router]);
