          ...
          <main className="min-h-[calc(100vh-14rem)] px-4 py-10">
            <div className="mx-auto flex w-full max-w-lg flex-col gap-4">
              <section className="solid-panel p-6 text-center">
                ...
                <p className="mx-auto mb-6 max-w-xs text-sm leading-6 text-[var(--color-muted)]">
                  ノートやプリントを撮影して、最初の単語帳を作りましょう。
                </p>
                {isAuthenticated ? (
                  <button
                    onClick={() => handleScanButtonClick(false)}
                    className="solid-link-primary mx-auto w-full max-w-xs"
                  >
                    <Icon name="photo_camera" size={20} />
                    スキャンをはじめる
                  </button>
                ) : (
                  <Link href="/signup" className="solid-link-primary mx-auto w-full max-w-xs">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Link href="/correction" className="solid-panel group p-4 transition-transform hover:-translate-y-0.5">
                  <div className="mb-3 flex h-11 w-11 items-center justify-center rounded-[16px] border-2 border-[var(--solid-ink)] bg-[var(--color-amber)]">
                    <Icon name="auto_awesome" size={24} className="text-[var(--solid-ink)]" />
                  </div>
                  <p className="text-sm font-black text-[var(--solid-ink)]">英文添削</p>
                  <p className="mt-1 text-xs leading-5 text-[var(--color-muted)]">書いた英文を自然な表現に整えます</p>
                </Link>
                <Link href="/parser" className="solid-panel group p-4 transition-transform hover:-translate-y-0.5">
                  <div className="mb-3 flex h-11 w-11 items-center justify-center rounded-[16px] border-2 border-[var(--solid-ink)] bg-[var(--color-sky)]">
                    <Icon name="account_tree" size={24} className="text-[var(--solid-ink)]" />
                  </div>
                  <p className="text-sm font-black text-[var(--solid-ink)]">構造解析</p>
                  <p className="mt-1 text-xs leading-5 text-[var(--color-muted)]">長文のSVOと節を見える化します</p>
                </Link>
              </div>

              {!isAuthenticated && (
                <p className="text-center text-xs text-[var(--color-muted)]">
                  登録すると単語帳と学習履歴をクラウドに保存できます
                </p>
              )}
            </div>
          </main>
      ...
      <div className="flex flex-col pb-28 lg:pb-8">
        ...
        <main className="flex-1 max-w-lg lg:max-w-3xl xl:max-w-5xl mx-auto px-4 lg:px-8 pt-4 pb-8 w-full space-y-5">
          ...
          <section className="grid gap-3 sm:grid-cols-2">
            <Link href="/correction" className="card p-4 transition-opacity active:opacity-80">
              <div className="flex items-start gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--color-foreground)] text-white">
                  <Icon name="edit_note" size={22} />
                </span>
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <h2 className="font-bold text-[var(--color-foreground)]">添削</h2>
                    <span className="rounded-[3px] bg-[var(--color-accent)] px-1.5 py-0.5 font-mono text-[9px] font-black text-white">NEW</span>
                  </div>
                  <p className="mt-1 text-xs leading-5 text-[var(--color-muted)]">英文を赤ペンで直し、ミスを復習へつなげます。</p>
                </div>
              </div>
            </Link>
            <Link href="/parser" className="card p-4 transition-opacity active:opacity-80">
              <div className="flex items-start gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--color-foreground)] text-white">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <h2 className="font-bold text-[var(--color-foreground)]">構造解析</h2>
                    <span className="rounded-[3px] bg-[var(--color-accent)] px-1.5 py-0.5 font-mono text-[9px] font-black text-white">NEW</span>
                  </div>
                  <p className="mt-1 text-xs leading-5 text-[var(--color-muted)]">長文をSVOと節に分けて骨格を見ます。</p>
                </div>
              </div>
            </Link>
          </section>
