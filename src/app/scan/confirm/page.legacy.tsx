...
export default function ConfirmPage() {
  ...
  if (!dataReady) {
    return (
      <div className="min-h-screen bg-[var(--color-background)] flex items-center justify-center">
        <SolidPanel className="w-full max-w-sm p-6">
  ...
  return (
    <SolidPage maxWidth="max-w-lg" className="pb-32">
      <SolidHeader
        eyebrow="SCAN RESULT"
        title={isAddingToExisting ? '追加する単語を確認' : '確認・編集'}
        description={`${selectedCount}語選択中${!isPro ? ` / 残り${availableSlots}語` : ''}`}
        backHref="/scan"
        actions={
          <button
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[var(--color-surface)] border-t border-[var(--color-border)] px-5 py-4 safe-area-bottom z-40">
        <div className="max-w-lg mx-auto">
          <button
            ...
            className="solid-link-primary w-full disabled:opacity-50"
          >
      ...
      <div className="bg-[var(--color-surface-secondary)] p-4">
        <div className="space-y-3">
          <input
            ...
            className="solid-input w-full px-3 py-2.5 text-base"
            autoFocus
            placeholder="英単語"
          />
          <input
            ...
            className="solid-input w-full px-3 py-2.5 text-sm"
            placeholder="日本語訳"
          />
