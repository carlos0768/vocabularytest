...
async function resolveProjectId(
  ...
  const title = `添削保存 ${new Date().toLocaleDateString('ja-JP')}`;
  const { data, error } = await supabase
    .from('projects')
    .insert({ user_id: userId, title, source_labels: ['correction'] })
    .select('id')
    .single();
...
export async function POST(request: NextRequest, context: Params) {
  try {
    ...
    const result = row.result as CorrectionResultPayload;
    ...
    console.error('[correction/save-words] failed:', error);
    return NextResponse.json({ success: false, error: '単語の保存に失敗しました' }, { status: 500 });
  }
}
