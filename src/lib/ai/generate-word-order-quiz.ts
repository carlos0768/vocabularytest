...
ルール:
- answerTokens は正解として選ばせる英単語チップ。必ず元の english に含まれる語だけを、正しい順番で最大3つ返す
- english が4語以上なら、4語目以降など選ばせない語は sentenceTokens の固定語として本文内に組み込む
- sentenceTokens は表示する英文トークン配列。answerTokens の位置には必ず "${WORD_ORDER_BLANK_TOKEN}" を入れる
