...
import {
  ...
  HIGHLIGHTED_WORD_USER_PROMPT_MULTI,
} from './prompts';
...
// Extracts highlighted words from multiple images in a single API call.
// All images are sent together so the AI can unify marker color calibration across pages.
// Falls back to the single-image function when only 1 image is provided.
export async function extractHighlightedWordsFromImages(
  imagesBase64: string[],
  apiKey: string,
  openaiApiKey?: string
): Promise<HighlightedExtractionResult> {
  // Fallback to single-image function for 1 image
  if (imagesBase64.length === 1) {
    return extractHighlightedWordsFromImage(imagesBase64[0], apiKey, openaiApiKey);
  }
  ...
  // Parse all images into { base64, mimeType } objects
  const parsedImages: Array<{ base64: string; mimeType: string }> = [];
  for (const imageBase64 of imagesBase64) {
    ...
    parsedImages.push({ base64: base64Data, mimeType });
  }

  console.log('AI API call (highlighted mode, multi-image):', {
    imageCount: parsedImages.length,
    totalBase64Length: parsedImages.reduce((sum, img) => sum + img.base64.length, 0),
    confidenceThreshold: CONFIDENCE_THRESHOLD,
  });
  ...
  try {
    ...
    const result = await provider.generate({
      systemPrompt: HIGHLIGHTED_WORD_EXTRACTION_SYSTEM_PROMPT,
      prompt: HIGHLIGHTED_WORD_USER_PROMPT_MULTI,
      images: parsedImages,
      config: {
        ...config,
        temperature: 0.5,
    ...
    console.log('AI raw response (highlighted mode, multi-image):', content.slice(0, 500) + '...');
    ...
    try {
      ...
      console.error('Failed to parse AI response (multi-image):', content);
      return { success: false, error: 'AIの応答を解析できませんでした' };
    }
    ...
    if (!validated.success) {
      console.error('Schema validation failed (multi-image):', validated.error);
      return {
        success: false,
        error: validated.error || 'データ形式が不正です',
      };
    ...
    console.log('Highlighted word detection metadata (multi-image):', {
      totalWordsDetected: highlightedData.words.length,
      detectedColors: highlightedData.detectedColors,
      totalHighlightedRegions: highlightedData.totalHighlightedRegions,
      imageCount: parsedImages.length,
    ...
    console.log('Confidence filtering result (multi-image):', {
      beforeFilter: highlightedData.words.length,
      afterFilter: filteredWords.length,
      threshold: CONFIDENCE_THRESHOLD,
    });
    ...
    console.log('Duplicate removal result (multi-image):', {
      beforeDedup: filteredWords.length,
      afterDedup: uniqueWords.length,
    });
    ...
    console.error('AI API error (highlighted mode, multi-image):', error);
