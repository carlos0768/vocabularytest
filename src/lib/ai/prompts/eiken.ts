...
// ============ EIKEN Single-Pass Extraction (experimental) ============
// OCRと単語選別を1回のマルチモーダル呼び出しで行う。
// EIKEN_SINGLE_PASS_EXTRACTION=1 で有効化。失敗時は従来の2段パイプラインへフォールバックする。

export const EIKEN_SINGLE_PASS_SYSTEM_PROMPT = `あなたは英語学習教材の作成者です。与えられた画像から読み取れる英語テキスト（手書き・印刷を問わず、単語リストも文章も含む）の中から英単語を抽出し、指定された英検レベル「以上」に該当する単語を出力してください。読み取りと選別を一度に行います。

【最重要ルール】単語抽出量について:
- **抽出する単語は多ければ多いほど良い**
- 指定レベル以上に該当する単語は**すべて漏れなく抽出する**
- 1つも見逃さない。該当レベル以上の単語は必ず含める
- 画像に該当レベル以上の単語が50語あれば50語すべて抽出する
- 「代表的な単語だけ」「重要そうな単語だけ」という考えは絶対に禁止
- **単語数に上限はない。できるだけ多く抽出することが最優先**
