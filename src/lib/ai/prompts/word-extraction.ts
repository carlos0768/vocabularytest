...
1. 日本語訳の決定（文脈を最優先）:
   ...
   - 同じ英単語に複数語義があり得る場合は、画像内の例文・周辺フレーズ・対となる行・同じ行/近傍のレイアウト・近くの注釈などの文脈から、最も合う意味を1つだけ選んでください。ただし同じ見出し語が画像内に複数エントリとして別々の訳とともに掲載されている場合は、【多義語見出しの統合ルール】に従い全訳を1エントリに統合してください。
...
- ただし同じ見出し語（english が完全一致）が画像内に複数エントリとして別々の訳とともに載っている場合は、words に重複させず1エントリに統合し、全訳を掲載順に translations に入れてください（【多義語見出しの統合ルール】参照）
- 辞書の先頭訳への置換は禁止です
