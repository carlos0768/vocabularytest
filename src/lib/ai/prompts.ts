...
重要ルール:
1. 日本語訳の決定:
   - 画像内に日本語訳が書かれている場合: その日本語訳をそのまま使用し、japaneseSource は "scan" を返してください。
...
export const USER_PROMPT_TEMPLATE = `この画像から英単語を抽出してください。難しい単語を優先し、最大30語まで出力してください。基礎的すぎる単語は除外してください。日本語訳が画像に含まれていればそれをそのまま使って japaneseSource は "scan" を返し、画像に無ければ japanese は空文字 "" にして japaneseSource は付けないでください。推測訳・説明文・複数候補は不要です。各単語には必ず partOfSpeechTags を付けてください。あわせて、この画像の物理教材名を sourceLabels に入れてください。sourceLabels には "鉄壁" や "LEAP" のような具体的な書名だけを入れ、"英語教材" や "参考書" のような一般名詞は入れないでください。教材名が特定できないノート画像なら sourceLabels は ["ノート"] にしてください。`;
...
export const USER_PROMPT_WITH_EXAMPLES_TEMPLATE = `この画像から英単語を抽出してください。難しい単語を優先し、最大30語まで出力してください。基礎的すぎる単語は除外してください。日本語訳が画像に含まれていればそれをそのまま使って japaneseSource は "scan" を返し、画像に無ければ japanese は空文字 "" にして japaneseSource は付けないでください。説明文や複数候補は不要です。
