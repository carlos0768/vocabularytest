...
export const USER_PROMPT_TEMPLATE = `この画像から英単語を抽出してください。難しい単語を優先し、最大30語まで出力してください。基礎的すぎる単語は除外してください。日本語訳が画像に含まれていればそれを使い、なければ適切な日本語訳を生成してください。各単語には必ず partOfSpeechTags を付けてください。あわせて、この画像の物理教材名を sourceLabels に入れてください。sourceLabels には "鉄壁" や "LEAP" のような具体的な書名だけを入れ、"英語教材" や "参考書" のような一般名詞は入れないでください。教材名が特定できないノート画像なら sourceLabels は ["ノート"] にしてください。`;
...
丸が日本語側にある場合は、対応する英単語と日本語訳を返してください。日本語訳が画像にない場合のみ自然な訳を補ってください。sourceLabels には物理教材名だけを入れ、"鉄壁" や "LEAP" のような具体的書名だけを返してください。"英語教材" や "参考書" のような一般名詞は返さず、教材名不明のノート画像なら ["ノート"] を返してください。`;
...
export const IDIOM_USER_PROMPT = `この画像から、動詞を含む熟語・句動詞・定型コロケーションのうち「意味を知らないと理解しにくい表現」だけを抽出してください。条件に合わない通常の語句は除外してください。日本語訳が画像に含まれていればそれを使い、なければ適切な日本語訳を生成してください。sourceLabels には物理教材名だけを入れ、"鉄壁" や "LEAP" のような具体的書名だけを返してください。"英語教材" や "参考書" のような一般名詞は返さず、教材名不明のノート画像なら ["ノート"] を返してください。`;
