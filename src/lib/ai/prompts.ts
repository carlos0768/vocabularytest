...
見出し語エントリの判定基準：番号（0001, 0002...）やチェックボックス（□）が先頭にある行のフレーズ。
例文（インデントされた下位テキスト行）中の単語には このルールを適用しない。
...
【見出し語フレーズのルール】
- 単語帳の番号付き・チェックボックス付き見出し語エントリとして複数単語のフレーズが掲載されている場合、そのフレーズ全体を english にしてください。太字・赤字の単語だけを取り出すのは禁止です。
...
- 例: 見出し "require further **validation**" → english: "require further validation" ✅ / english: "validation" ❌
