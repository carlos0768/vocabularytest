...
✓ **手書きのアンダーライン（下線）が引かれた単語** → **線のすぐ上にある単語のみ**を抽出する

✗ **アンダーラインの下の行にある単語**（線の下に位置する別の単語は対象外。線の上の単語だけを抽出する）
...
【重要：アンダーラインの「上の単語」とは】
- アンダーラインは単語の**下に引かれる線**である
- その線が引かれている**直上の単語**（つまり下線が付いている単語）を抽出する
- 線の**下の行にある別の単語**は対象外（紛らわしいが、線と単語の位置関係で判断する）
- 例: "apple" の下にアンダーラインがあり、その下の行に "banana" がある場合 → "apple" のみ抽出
