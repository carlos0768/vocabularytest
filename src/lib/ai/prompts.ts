...
2. 物理ソース名の判定:
   ...
   - 特定の書籍名・教材名が読み取れる場合はその名前をそのまま入れてください（例: "鉄壁"）。
...
- 英単語のみの画像でも、必ず日本語訳を生成して出力してください。${SOURCE_LABEL_NOTES}`;

export const USER_PROMPT_TEMPLATE = `この画像から英単語を抽出してください。難しい単語を優先し、最大30語まで出力してください。基礎的すぎる単語は除外してください。日本語訳が画像に含まれていればそれを使い、なければ適切な日本語訳を生成してください。各単語には必ず partOfSpeechTags を付けてください。あわせて、この画像の物理教材名を sourceLabels に入れてください。教材名が特定できないノート画像なら sourceLabels は ["ノート"] にしてください。`;
...
export const USER_PROMPT_WITH_EXAMPLES_TEMPLATE = `この画像から英単語を抽出してください。難しい単語を優先し、最大30語まで出力してください。基礎的すぎる単語は除外してください。日本語訳が画像に含まれていればそれを使い、なければ適切な日本語訳を生成してください。
...
- sourceLabels: 物理教材名の配列。教材名不明のノート画像なら ["ノート"]`;
...
丸が日本語側にある場合は、対応する英単語と日本語訳を返してください。日本語訳が画像にない場合のみ自然な訳を補ってください。sourceLabels には物理教材名だけを入れ、教材名不明のノート画像なら ["ノート"] を返してください。`;
...
export const IDIOM_USER_PROMPT = `この画像から、動詞を含む熟語・句動詞・定型コロケーションのうち「意味を知らないと理解しにくい表現」だけを抽出してください。条件に合わない通常の語句は除外してください。日本語訳が画像に含まれていればそれを使い、なければ適切な日本語訳を生成してください。sourceLabels には物理教材名だけを入れ、教材名不明のノート画像なら ["ノート"] を返してください。`;
...
日本語訳が画像にあればそれを使い、なければ生成してください。
partOfSpeechTags には noun / verb / adjective / adverb / idiom / phrasal_verb など、最も適切な分類を1つ以上入れてください。${SOURCE_LABEL_NOTES}`;

export const HIGHLIGHTED_WORD_USER_PROMPT = `この画像で、手書きのマーカーやペンで印をつけた単語だけを抽出してください。印のない単語は含めないでください。sourceLabels には物理教材名だけを入れ、教材名不明のノート画像なら ["ノート"] を返してください。`;
