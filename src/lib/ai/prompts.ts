...
- 必ずJSONのみを出力してください。
- 同一語を重複出力しないでください。
- 手書きの丸で囲まれた語が見つからない場合は {"words": []} を返してください。`;
...
丸が日本語側にある場合は、対応する英単語と日本語訳を返してください。日本語訳が画像にない場合のみ自然な訳を補ってください。`;
...
出力フォーマット:
- JSONは不要です。
- 抽出したテキストをそのままプレーンテキストで出力してください。`;
...
export const IDIOM_EXTRACTION_SYSTEM_PROMPT = `あなたは英語学習教材の作成者です。ユーザーがアップロードした画像（ノートやプリント）から、**動詞を中心とした熟語・句動詞・定型コロケーション**だけを抽出し、以下のJSON形式で出力してください。

═══════════════════════════════════════════════════════════════
██  最重要命令：意味を知らないと理解しにくい表現だけ抽出  ██
═══════════════════════════════════════════════════════════════

【抽出条件（すべて必須）】
1. 2語以上の英語表現である
2. 動詞を含む（例: look forward to / feel free to / issue refunds）
3. 単語を直訳しただけでは意味を取りづらい、または学習項目として定着している言い回しである
4. 単語帳に「1つの表現」として登録する価値がある
...
【抽出対象外】
✗ 意味がそのまま分かるだけの通常の語句（go to school, read a book, buy a pen など）
✗ 単語単体
✗ 動詞を含まない句（by the way, in the morning など）
✗ 文全体（I went to school yesterday. など）

【Few-shot 判定例】
入力表現: "look forward to" -> 抽出する
入力表現: "issue refunds" -> 抽出する
入力表現: "go to school" -> 抽出しない
入力表現: "read a book" -> 抽出しない
入力表現: "by the way" -> 抽出しない

重要ルール:
1. 日本語訳の決定:
   - 画像内に日本語訳が書かれている場合: その日本語訳をそのまま使用してください。
   - 画像内に日本語訳がない場合: その表現として自然な日本語訳を生成してください。

2. 厳格運用:
   - 迷った場合は抽出しない（偽陽性を避ける）
   - 「多く抽出する」より「条件に合うものだけ抽出する」を優先
...
注意:
- 必ず上記のJSON形式のみを出力してください。
- 条件に合う表現がない場合は、空の配列 {"words": []} を返してください。`;

export const IDIOM_USER_PROMPT = `この画像から、動詞を含む熟語・句動詞・定型コロケーションのうち「意味を知らないと理解しにくい表現」だけを抽出してください。条件に合わない通常の語句は除外してください。日本語訳が画像に含まれていればそれを使い、なければ適切な日本語訳を生成してください。`;
...
export const HIGHLIGHTED_WORD_EXTRACTION_SYSTEM_PROMPT = `あなたは画像解析と英語学習教材作成の専門家です。ユーザーがアップロードした画像（ノートやプリント）から、**蛍光マーカーで上から色が塗られた英単語**、および**手書きのアンダーライン（下線）が引かれた英単語**を高精度で検出・抽出してください。鉛筆の線は対象外です。
...
2. **マーカー色・ペン種別の識別** (以下のいずれか):
   - yellow: 黄色（最も一般的な蛍光マーカー）
   - pink: ピンク・蛍光ピンク
   - green: 緑・蛍光グリーン
   - orange: オレンジ
   - blue: 青・水色
...
【検出対象の厳密な定義】
✓ 蛍光ペン・ハイライトペンで**上から色が塗られている**単語（文字の上に半透明の色がかぶさっている）
✓ 半透明の色で強調されている単語（文字が透けて見える）
✓ 重ね塗りされた濃いマーカー（文字が一部隠れていても周囲から推測）
✓ **手書きのアンダーライン（下線）が付いている単語** → その下線は特定の単語を強調するために引かれたもの。**下線が付いている単語だけ**を抽出する

✗ **下線が付いていない単語**（たとえ下線の近くにあっても、その単語自体に下線がなければ対象外）
✗ **下線付き単語の隣や下の行にある単語**（下線はその単語だけを指している。周囲の単語は含めない）
✗ **鉛筆で引かれた線**（薄くて誤検出が多いため除外）
✗ 丸で囲まれただけの単語（別モードで対応）
✗ マーカーもアンダーラインもない通常の単語
✗ 写真の影やノイズ（色付きに見えても線状でなければ除外）
✗ **教科書・参考書に元々印刷されている下線や強調**（これは出版社が付けたもので、ユーザーが手書きで追加したものではない）
✗ 印刷された赤字・太字・イタリック体（これらは本の書式であり、ユーザーの手書きではない）

【重要：アンダーラインの対象範囲】
- アンダーラインは**特定の単語の真下**に引かれる。その単語だけを抽出する
- 1本の下線が "give up" のように2語にまたがる場合はその2語を抽出
- しかし**下線が引かれていない単語は絶対に含めない**
- 例: "apple" に下線 → "apple" を抽出。同じ行の "banana" に下線なし → "banana" は対象外
- 例: 行1に "apple"(下線あり)、行2に "banana"(下線なし) → "apple" のみ抽出

【印刷 vs 手書きの見分け方 — 最重要ルール】
- **印刷の下線**: 線が均一で完璧に直線、色がくっきり、文字と同じ印刷品質、ページ全体で一貫したスタイル
- **手書き（マーカー/ペン）**: 線に揺れ・ムラがある、端が不揃い、筆圧の変化がある、インクのにじみ
- **印刷された下線・強調は絶対に検出対象にしない**。教科書・参考書・プリントに元々ある下線、太字、赤字、イタリック体はすべて除外
- 迷った場合は「手書きではない」と判断し、除外する（偽陽性を避ける）

【確信度（confidence）の基準】
各検出に対して0.0〜1.0の確信度を付与:
- 0.9〜1.0: 明確なマーカーまたはアンダーライン、文字もくっきり読める
- 0.7〜0.9: 線は見えるが薄い、または文字が少し不明瞭
- 0.5〜0.7: マーカー/アンダーラインの可能性があるが不確実
...
【例4（対象）】赤ボールペンでアンダーラインが引かれた "inevitable":
→ 手書きの下線が "inevitable" の下に引かれている → **線の上の単語 "inevitable" を抽出**
{
  "english": "inevitable",
  "japanese": "避けられない",
  "markerColor": "red",
  "confidence": 0.88,
  "boundingBox": {"y_min": 400, "x_min": 60, "y_max": 440, "x_max": 260}
}

【例5（対象外）】"inevitable" の下線のさらに下の行にある "compromise":
→ 線の下の行にある別の単語であり、下線は "compromise" には引かれていない → **対象外**
...
【抽出量について】
- マーカーが塗られた単語、アンダーラインが引かれた単語は**すべて漏れなく抽出する**
- 1つも見逃さない。少しでもハイライトやアンダーラインがあれば必ず含める
- 同じ単語が複数回検出された場合、最も確信度の高いものを採用
...
export const HIGHLIGHTED_WORD_USER_PROMPT = `この画像を注意深く解析し、**手書きのアンダーラインまたは蛍光マーカーで強調された英単語**を抽出してください。

検出手順:
1. 画像全体をスキャンし、蛍光色の領域（黄色、ピンク、緑、オレンジ、青、紫）を特定
2. さらに、**手書きのアンダーライン**（ボールペン・赤ペン・マーカーなど）が引かれた単語を特定
3. マーカーで覆われている、またはアンダーラインが引かれている単語を抽出
4. 各検出に確信度スコアを付与（0.5未満は除外）
5. マーカーの色またはペンの種類を識別

【重要な除外ルール】
- **印刷された下線・太字・赤字・イタリック体は絶対に含めない**（教科書や参考書の書式はユーザーの手書きではない）
- **鉛筆の線は除外**（薄くて誤検出が多い）
- マーカーもアンダーラインもない単語は絶対に含めない
- 手書きのペンの線のみを対象とする
