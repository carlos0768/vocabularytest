...
1. 日本語訳の決定:
   ...
   - 画像内に日本語訳がない場合（英単語のみの場合）: japanese は空文字 "" を返してください。日本語訳を推測・生成してはいけません。
${SOURCE_LABEL_RULES}
...
- 画像に日本語訳が無い場合は japanese に空文字 "" を入れてください。${SOURCE_LABEL_NOTES}`;

export const USER_PROMPT_TEMPLATE = `この画像から英単語を抽出してください。難しい単語を優先し、最大30語まで出力してください。基礎的すぎる単語は除外してください。日本語訳が画像に含まれていればそれを使い、画像に無ければ japanese は空文字 "" を返してください。訳の推測生成はしないでください。各単語には必ず partOfSpeechTags を付けてください。あわせて、この画像の物理教材名を sourceLabels に入れてください。sourceLabels には "鉄壁" や "LEAP" のような具体的な書名だけを入れ、"英語教材" や "参考書" のような一般名詞は入れないでください。教材名が特定できないノート画像なら sourceLabels は ["ノート"] にしてください。`;
...
- 必ず上記のJSON形式のみを出力してください。
- 画像から英単語が読み取れない場合は、空の配列 {"words": []} を返してください。
- 英単語のみの画像では japanese は空文字 "" を返し、例文だけ生成してください。
- **partOfSpeechTags は必須です。必ず1つの主分類を配列で返してください。**
- partOfSpeechTags は次のいずれか1つだけを使ってください:
  noun, verb, adjective, adverb, idiom, phrasal_verb, preposition, conjunction, pronoun, determiner, interjection, auxiliary, other
- 熟語は idiom、句動詞は phrasal_verb を優先してください。
...
export const USER_PROMPT_WITH_EXAMPLES_TEMPLATE = `この画像から英単語を抽出してください。難しい単語を優先し、最大30語まで出力してください。基礎的すぎる単語は除外してください。日本語訳が画像に含まれていればそれを使い、画像に無ければ japanese は空文字 "" を返してください。訳の推測生成はしないでください。
...
1. 日本語訳の決定:
   ...
   - テキスト内に日本語訳がない場合: japanese は空文字 "" を返してください。日本語訳を推測・生成してはいけません。
...
1. 日本語訳の決定:
   - 画像内に日本語訳が書かれている場合: その日本語訳をそのまま使用してください。
   - 画像内に日本語訳がない場合: japanese は空文字 "" を返してください。日本語訳を推測・生成してはいけません。
${SOURCE_LABEL_RULES}
...
日本語訳が画像にあればそれを使い、無ければ japanese は空文字 "" を返してください。推測生成はしないでください。
