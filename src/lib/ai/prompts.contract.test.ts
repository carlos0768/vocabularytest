...
test('extraction prompts keep lemma normalization with independent-meaning exception', () => {
  assertIncludesAll('LEMMA_NORMALIZATION_RULES', LEMMA_NORMALIZATION_RULES, [
    '単語の原形化ルール',
  ...
  for (const [name, prompt] of prompts) {
    assertIncludesAll(name, prompt, [LEMMA_NORMALIZATION_RULES]);
  }

  // 検証プロンプトは原形化された候補を活用形の印と同一視できること
  for (const [name, prompt] of [
    ['CIRCLED_WORD_VERIFICATION_SYSTEM_PROMPT', CIRCLED_WORD_VERIFICATION_SYSTEM_PROMPT],
    ['HIGHLIGHTED_WORD_VERIFICATION_SYSTEM_PROMPT', HIGHLIGHTED_WORD_VERIFICATION_SYSTEM_PROMPT],
  ] as const) {
    assertIncludesAll(name, prompt, ['候補語は原形化されている場合があります']);
  }
});

test('EIKEN single-pass prompt keeps level filtering and extraction-volume rules', () => {
  assertIncludesAll('EIKEN_SINGLE_PASS_SYSTEM_PROMPT', EIKEN_SINGLE_PASS_SYSTEM_PROMPT, [
    '{LEVEL_DESC}「以上」に相当する単語',
    '対象レベル: {LEVEL_RANGE}',
    '指定レベル未満の単語は1語も出力しない',
    'レベル判定に迷う単語は安全側で除外',
