...
interface SendHelloWorldEmailParams {
  to?: string;
}
...
export async function sendHelloWorldEmail({
  to = 'andyui_51@icloud.com',
}: SendHelloWorldEmailParams = {}): Promise<ResendResponse> {
  const resend = getResendClient();
  if (!resend) {
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: 'Merken <noreply@merken.jp>',
      to,
      subject: 'Merken Proプランへの変更が完了しました',
      html: `
        <p>Merkenをご利用いただきありがとうございます。</p>
        <p>お客様のプランが<strong>Proプラン</strong>に変更されました。</p>
        <p>Pro機能を引き続きご利用いただけます。</p>
      `,
      text: 'Merkenをご利用いただきありがとうございます。\n\nお客様のプランがProプランに変更されました。\nPro機能を引き続きご利用いただけます。',
    });
