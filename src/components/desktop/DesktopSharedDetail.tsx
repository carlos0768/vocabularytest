...
export function DesktopSharedDetailView({
  ...
  /** True only for logged-out visitors: shared wordbooks are open to all logged-in users. */
                    ...
                    {selectMode && (
                      <td>
                        <span className={'ds-check' + (selected ? ' on' : '')}>
                          {selected && <Icon name="check" style={{ fontSize: 15, color: 'var(--color-on-accent)' }} />}
                        </span>
                      </td>
                    )}
                    <td className="en">
